...
# Date 11.04
rm(list=ls())
directory = "/Users/alex.garcia/Desktop/FHS/Coaching/Data/November/Billing/2019-12-01"  # Enter the directory where the data for a specific date is stored
date.first.day = "12_01_2019"                            # Enter the date of the first day of month of the current data with format "mm_dd/_YYY"
date.current.report = "12_01_2019"                       # Enter the date of the current data with format "mm_dd_YYYY"
...
clean_merge_data <- function(dir = directory, first.day = date.first.day, date.current = date.current.report){
  ...
  # Then categorize via indicators
  data_all$Group.GA <- ifelse(str_detect(data_all$Group.All, "GA"), 1, 0)
  data_all$Group.TX <- ifelse(str_detect(data_all$Group.All, "TX") & !str_detect(data_all$Group.All, "FIT"), 1, 0)
  data_all$Group.FIT <- ifelse(str_detect(data_all$Group.All, "FIT"), 1, 0)
  data_all$Group.NCSC <- ifelse(str_detect(data_all$Group.All, "NCSC") & !str_detect(data_all$Group.All, "CGM"), 1, 0)

  # Finally deal with NCSC.CGM
  # Create indicator for removal conditions
  data_all$Group.CGM.Remove <- ifelse(str_detect(data_all$Group.All, "DROPOFF")  |
                                      str_detect(data_all$Group.All, "DECLINED") |
                                      str_detect(data_all$Group.All, "ENDED"), 1, 0)

  # Create indicator for any occurrence of "CGM"
  data_all$Group.CGM <- ifelse(str_detect(data_all$Group.All, "CGM"), 1, 0)

  # Create indicator for true CGM
  data_all$Group.CGM.True <- ifelse(data_all$Group.CGM == 1 & data_all$Group.CGM.Remove == 0, 1, 0)

  # Make an indicator for someone who has "FIT" in any Group column
  data_all$Group.FIT <- ifelse(str_detect(data_all$Group.All, "FIT"), 1, 0)

  # Make an indicator JOY tags in any Group column
  data_all$Group.JOY <- ifelse(str_detect(data_all$Group.All, "JBT") |
                               str_detect(data_all$Group.All, "JBC"), 1, 0)
  ...
  data_all$Group <- ifelse(data_all$Group.CGM.True == 1, "CGM", data_all$Group) # We've already removed declined/dropoff/ended folks above
  data_all$Group <- ifelse(data_all$Group.FIT == 1, "FIT", data_all$Group)
  data_all$Group <- ifelse(data_all$Group.JOY == 1, "JOY", data_all$Group) # Not sure why it's necessary to specify != "FIT here
